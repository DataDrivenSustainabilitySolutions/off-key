...
def register(user: users.UserCreate, db: Session = Depends(get_db_sync)):
    ...
    verification_token = auth_services.create_verification_token(new_user.email, expires_minutes=15)
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db_sync)):
