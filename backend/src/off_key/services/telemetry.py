...
class TelemetrySyncService:
    ...
    async def sync_telemetry(self, limit: int):
        ...
        log_ids_display = all_charger_ids[:5] + (['...'] if len(all_charger_ids) > 5 else [])
        ...
        for charger_id in all_charger_ids:
            ...
            logger.info(f"--- Processing Charger ID: {charger_id} ({chargers_processed_count}/{len(all_charger_ids)}) ---")
            ...
            try:
                ...
                if not isinstance(device_model, dict):
                     logger.warning(f"Received unexpected device model format for {charger_id}. Skipping.")
                     continue
            except Exception as e:
            ...
            if not telemetry_hierarchies:
                logger.warning(f"No telemetry hierarchies found or extracted for charger {charger_id}.")
                continue
            ...
            for hierarchy_raw in telemetry_hierarchies:
                ...
                logger.debug(f"Processing hierarchy {hierarchies_processed_count}/{len(telemetry_hierarchies)}: '{hierarchy_raw}' for {charger_id}.")

                hierarchy_url_part = hierarchy_raw.replace("/", "%2F")
                hierarchy_db_type = clean_string(hierarchy_raw)

                if not hierarchy_db_type:
                     logger.warning(f"Skipping hierarchy '{hierarchy_raw}' for {charger_id} due to cleaning returning None.")
                     continue

                # Construct API URL WITHOUT date range ('dr').
                get_url = f"chargers/{charger_id}/telemetry/{hierarchy_url_part}?Limit={limit}"
                logger.info(f"Fetching ALL telemetry data points: {get_url}") # Log change

                try:
                    telemetry_api_response = await self.client.get(get_url)
                    items = telemetry_api_response.get("items", []) if isinstance(telemetry_api_response, dict) else []
                except Exception as e:
                     logger.error(f"Failed to fetch telemetry from {get_url}: {e}")
                     continue

                if not items:
                    # This message now means no data *at all* exists for this hierarchy (or none within the Limit)
                    logger.info(f"No telemetry items retrieved for {charger_id} / {hierarchy_raw} from {get_url}")
                    continue

                # Log message reflects potentially large amount of data
                logger.info(f"Retrieved {len(items)} items (potentially all historical) for {charger_id} / {hierarchy_raw}. Preparing for insertion.")
                ...
                for item in items:
                    ...
                    try:
                        ...
                        items_skipped_count += 1
                        logger.warning(f"Skipping item {items_processed_count}/{len(items)} for {charger_id}/{hierarchy_raw} due to processing error: {item_error}. Item data: {item}")
                        continue

                if items_skipped_count > 0:
                    logger.warning(f"Skipped {items_skipped_count}/{len(items)} items for {charger_id}/{hierarchy_raw} due to processing errors.")
                if not telemetry_records_to_insert:
                     logger.info(f"No valid records to insert for {charger_id} / {hierarchy_raw} after processing {len(items)} items.")
                     continue

                # 3d. Bulk insert valid records in batches using ON CONFLICT DO NOTHING
                logger.info(f"Attempting to insert {len(telemetry_records_to_insert)} processed records for {charger_id} / {hierarchy_raw} (duplicates will be skipped by DB).")
                batch_size = 5000
                batch_num = 0
                for i in range(0, len(telemetry_records_to_insert), batch_size):
                    batch_num += 1
                    batch = telemetry_records_to_insert[i : i + batch_size]
                    logger.debug(f"Preparing batch {batch_num} ({len(batch)} records) for {charger_id} / {hierarchy_raw}.")
                    try:
                        stmt = insert(Telemetry).values(batch)
                        stmt = stmt.on_conflict_do_nothing()
                        await self.session.execute(stmt)
                        await self.session.commit()
                        logger.debug(f"Committed batch {batch_num} for {charger_id} / {hierarchy_raw}.")
                    except IntegrityError as ie:
                        await self.session.rollback()
                        logger.error(f"IntegrityError during batch {batch_num} insert for {charger_id}/{hierarchy_raw}: {ie}. Rolling back batch.")
                    except Exception as e:
                        await self.session.rollback()
                        logger.error(f"Error during batch {batch_num} insert for {charger_id}/{hierarchy_raw}: {e}. Rolling back batch.")

            logger.info(f"--- Finished processing {hierarchies_processed_count} hierarchies for Charger ID: {charger_id} ---")

        logger.info(f"Telemetry synchronization process completed. Processed {chargers_processed_count} chargers.")
