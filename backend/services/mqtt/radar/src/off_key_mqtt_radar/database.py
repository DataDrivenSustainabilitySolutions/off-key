...
class DatabaseWriter:
    ...
    async def write_anomaly(self, result: AnomalyResult):
        ...
        if (len(self.write_queue) >= self.config.db_batch_size or
            (time.time() - self.last_write_time) > self.config.db_batch_timeout):
            await self._flush_batch()
    ...
    async def _retry_failed_batch(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with self.session_factory() as session:
                    ...
                    logger.info(f"Successfully retried writing {len(retry_batch)} records")
                    return
        ...
        logger.error(f"Dropping failed batch of {len(self.write_queue)} records after {max_retries} attempts")
        self.write_queue.clear()
        self.total_errors += 1
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if not self.config.db_write_enabled:
            return {
                "status": "disabled",
                "reason": "write_disabled_in_config"
            }
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            try:
                ...
                result = await session.execute(text("SELECT to_regclass('radar_anomalies')"))
                ...
                if not table_exists:
                    ...
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_anomalies_topic ON radar_anomalies(topic)"))
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_anomalies_charger_id ON radar_anomalies(charger_id)"))
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_anomalies_severity ON radar_anomalies(severity)"))
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_anomalies_is_anomaly ON radar_anomalies(is_anomaly)"))
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_anomalies_score ON radar_anomalies(anomaly_score)"))
                    await session.execute(text("CREATE INDEX IF NOT EXISTS idx_radar_service_metrics_timestamp ON radar_service_metrics(timestamp)"))
                ...
                try:
                    ...
                    logger.error(f"SQLAlchemy metadata creation also failed: {metadata_error}")
                    logger.warning("Could not create tables automatically. Please create them manually or via migrations.")
        
        logger.info("Database table verification completed")
        
    except Exception as e:
        logger.error(f"Failed to ensure database tables exist: {e}")
        logger.warning("Continuing without table creation - tables should be handled by migrations or manual setup")