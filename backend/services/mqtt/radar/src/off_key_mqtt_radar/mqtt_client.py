...
class RadarMQTTClient:
    ...
    def __init__(self, config: MQTTRadarConfig):
        ...
        logger.info(f"Initialized RADAR MQTT client for broker {config.broker_host}:{config.broker_port}")
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    if self.message_handler:
                        ...
                        logger.debug(f"No message handler set, dropping message from {message.topic}")
    ...
    async def _schedule_reconnect(self):
        ...
        logger.info(f"Scheduling reconnection attempt {self.reconnect_attempts} in {delay:.1f} seconds")
