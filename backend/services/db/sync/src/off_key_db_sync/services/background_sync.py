...
class BackgroundSyncService:
    ...
    async def start(self):
        ...
        if config.chargers_interval > 0:
            self.scheduler.add_job(
                self._sync_chargers,
                trigger=IntervalTrigger(seconds=config.chargers_interval),
                id="charger_sync",
                name="Charger Sync",
            ...
            logger.info(
                f"Charger sync scheduled every {config.chargers_interval} seconds",
        ...
        if config.telemetry_interval > 0:
            self.scheduler.add_job(
                ...
                trigger=IntervalTrigger(seconds=config.telemetry_interval),
                id="telemetry_sync",
                name="Telemetry Sync",
                max_instances=config.scheduler_max_instances,
                coalesce=True,
            ...
            logger.info(
                f"Telemetry sync scheduled every {config.telemetry_interval} seconds",
                extra=self._log_context,
            )

        # Add charger cleanup job
        if config.cleanup_enabled and config.cleanup_interval > 0:
            self.scheduler.add_job(
            ...
            logger.info(
                ...
                f"{config.cleanup_interval} seconds "
                f"(threshold: {config.cleanup_days_inactive} days)",
                extra=self._log_context,
            )
    ...
    async def _sync_telemetry(self):
        ...
        try:
            ...
            async with session_factory() as session:
                ...
                await sync_service.sync_telemetry(limit=config.telemetry_limit)
        ...
        try:
            ...
            async with session_factory() as session:
                ...
                await sync_service.clean_chargers(
                    days_inactive=config.cleanup_days_inactive
                )
            ...
            logger.info(
                ...
                extra={
                    **self._log_context,
                    ...
                    "days_threshold": config.cleanup_days_inactive,
                },
            )
