...
class ConfigWatcher:
    ...
    def __init__(self, config_file_path: str, reload_callback: Callable[[], Awaitable[None]]):
        ...
        if not self.config_file_path.exists():
            logger.warning(f"Configuration file does not exist: {self.config_file_path}")
    ...
    async def start(self):
        ...
        try:
            # Create handler
            self.handler = ConfigFileHandler(self.config_file_path, self._handle_config_change)
    ...
    def _is_file_readable(self) -> bool:
        ...
        try:
            return self.config_file_path.exists() and os.access(self.config_file_path, os.R_OK)
        except Exception:
            return False
...
class ConfigReloader:
    ...
    async def reload_config(self):
        ...
        try:
            ...
            old_config = self.service.config.dict() if hasattr(self.service, 'config') else {}
    ...
    async def _handle_config_changes(self, old_config, new_config):
        ...
        mqtt_changed = (
            old_config.broker_host != new_config.broker_host or
            old_config.broker_port != new_config.broker_port or
            old_config.use_tls != new_config.use_tls or
            old_config.use_auth != new_config.use_auth or
            old_config.username != new_config.username or
            old_config.api_key != new_config.api_key
        )
        
        if mqtt_changed:
            logger.info("MQTT connection settings changed - will require reconnection")
            # Note: Full MQTT reconnection would be complex and might require service restart
        ...
        if old_config.memory_limit_mb != new_config.memory_limit_mb:
            logger.info(f"Memory limit changed: {old_config.memory_limit_mb} -> {new_config.memory_limit_mb} MB")
