...
class AnomalyRecord(Base):
    ...
    processed_features = Column(JSON, nullable=True)  # Preprocessed features used for detection
    ...
    def __repr__(self):
        return f"<AnomalyRecord(id={self.id}, topic='{self.topic}', severity='{self.severity}', score={self.anomaly_score:.3f})>"
...
class ModelCheckpoint(Base):
    ...
    def __repr__(self):
        return f"<ModelCheckpoint(id={self.id}, model_type='{self.model_type}', processed_count={self.processed_count})>"
...
class ServiceMetrics(Base):
    ...
    def __repr__(self):
        return f"<ServiceMetrics(id={self.id}, timestamp={self.timestamp}, status='{self.service_status}')>"
...
class AnomalyResult:
    ...
    def to_db_record(self, model_type: str, model_version: str = "1.0") -> AnomalyRecord:
        ...
        return AnomalyRecord(
            ...
            message_timestamp=self.timestamp if isinstance(self.timestamp, datetime) else None,
            context=self.context,
        )
